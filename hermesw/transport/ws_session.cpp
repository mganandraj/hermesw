...
class ws_session : public std::enable_shared_from_this<ws_session>,
                   public web_socket_session_interface {
  ...
  std::function<void(std::shared_ptr<web_socket_session_interface>)>
      on_connected_func_;
 ...
 public:
  ...
  explicit ws_session(
      ...
      std::function<void(std::shared_ptr<web_socket_session_interface>)>
          on_connected_func)
      : resolver_(ioc), ws_(ioc), on_connected_func_(on_connected_func) {}
  ...
  explicit ws_session(
      ...
      std::function<void(std::shared_ptr<web_socket_session_interface>)>
          on_connected_func)
      : resolver_(ioc), ws_(std::move(socket)) {}
  ...
  void writer_thread_func() {
    while (true) {
      ...
      for (std::string text : write_queue_copy) {
        std::cout << "writer_thread_func" << text << std::endl;
  ...
  void close() {
    ws_.async_close(
        ...
        std::bind(
            &ws_session::on_close, shared_from_this(), std::placeholders::_1));
  }
  ...
  void run_server() {
    ws_.async_accept(boost::beast::bind_front_handler(
        &ws_session::on_accept, shared_from_this()));
  }
  ...
  void do_read() {
    ...
    ws_.async_read(
        ...
        boost::beast::bind_front_handler(
            &ws_session::on_read, shared_from_this()));
  }
 ...
 public:
  listener(
      ...
      : ioc_(ioc),
        acceptor_(ioc),
        socket_(ioc),
        on_connected_func_(on_connected_func) {
    boost::system::error_code ec;
  ...
  void do_accept() {
    acceptor_.async_accept(
        socket_,
        std::bind(
            &listener::on_accept, shared_from_this(), std::placeholders::_1));
  }
  ...
  void on_accept(boost::system::error_code ec) {
    if (ec) {
      ...
      auto session = std::make_shared<ws_session>(
          ioc_, std::move(socket_), on_connected_func_);
...
void create_web_socket_client(
    unsigned short port,
    std::function<void(std::shared_ptr<web_socket_session_interface>)>
        on_connected_func) {
  boost::asio::io_context ioc;
  std::make_shared<ws_session>(ioc, on_connected_func)
      ->run_client("127.0.0.1", "8888");
  ioc.run();
}

void do_server() {
  create_web_socket_server(
      8888, [](std::shared_ptr<web_socket_session_interface> connection) {
        connection->setOnRead([](std::string text) {
          std::cout << "<< do_server :: Read Handler >> :: " << text
                    << std::endl;
        });
...
void do_client() {
  create_web_socket_client(
      8888, [](std::shared_ptr<web_socket_session_interface> connection) {
        connection->setOnRead([](std::string text) {
          std::cout << "<< do_client :: Read Handler >> :: " << text
                    << std::endl;
        });
...
//  }
//  else {
//    std::make_shared<ws_session>(ioc)->run("127.0.0.1", argv[3], "Hello how
//    are you ?");
//  }*/
//
//  // ioc.run();
